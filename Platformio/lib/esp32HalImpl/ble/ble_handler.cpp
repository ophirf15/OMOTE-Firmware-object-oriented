...
static bool sClassicBtMemReleased = false;

static void bleShutdown();
static void resetBtControllerToIdle();
static void bleSendKey(const String &keyName);
...
static bool bleSetProfile(const String &profileKey) {
...
/** Leave the ESP BT controller fully deinited so NimBLE can init cleanly. */
static void resetBtControllerToIdle() {
...
static void bleInit() {
  ...
  if (ESP.getFreeHeap() < kMinHeapForBleInit) {
    Serial.printf("BLE: init skipped — heap %u < %u\n", ESP.getFreeHeap(), kMinHeapForBleInit);
    return;
  }
  if (!sClassicBtMemReleased) {
    esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT);
    sClassicBtMemReleased = true;
  }
...
void bleShutdown() {
  ...
  sOpenAdvUntilMs = 0;
#ifndef IS_SIMULATOR
  Serial.printf("BLE: shutdown heap=%u status=%d\n", ESP.getFreeHeap(),
...
void bleStartPairingMode() {
  if (!sBleStarted)
    bleInit();
  if (!sBleStarted) {
    Serial.println("BLE: pairing aborted — stack not running");
    return;
  }
  sPairingMode = true;
...
void BleHandler::setProfile(const std::string &profileKey) {
  bleSetProfile(String(profileKey.c_str()));
}

std::string BleHandler::currentProfile() const {
  return std::string(bleCurrentProfile().c_str());
}

std::string BleHandler::identityListJson() const {
  return std::string(bleIdentityListJson().c_str());
}

void BleHandler::init() { bleInit(); }
void BleHandler::shutdown() { bleShutdown(); }
bool BleHandler::isInitialized() const { return bleIsInitialized(); }
bool BleHandler::isConnected() const { return bleIsConnected(); }
bool BleHandler::isAdvertising() const { return bleIsAdvertising(); }
bool BleHandler::isPairingMode() const { return bleIsPairingMode(); }
void BleHandler::startAdvertising() { bleStartAdvertising(); }
void BleHandler::stopAdvertising() { bleStopAdvertising(); }
void BleHandler::onWake(uint32_t windowMs) { bleOnWake(windowMs); }
void BleHandler::startPairingMode() { bleStartPairingMode(); }
void BleHandler::stopPairingMode() { bleStopPairingMode(); }
void BleHandler::disconnectClients() { bleDisconnectClients(); }
void BleHandler::forgetBonds() { bleForgetBonds(); }
void BleHandler::sendKey(const std::string &keyName) { bleSendKey(String(keyName.c_str())); }
void BleHandler::sendText(const std::string &text) { bleSendText(String(text.c_str())); }
bool BleHandler::sendRawConsumerUsage(uint16_t usage) { return bleSendRawConsumerUsage(usage); }
bool BleHandler::sendRawButton(uint8_t button1to16) { return bleSendRawButton(button1to16); }
void BleHandler::taskLoop() { bleTaskLoop(); }
std::string BleHandler::statusJson() const { return std::string(bleStatusJson().c_str()); }

std::shared_ptr<BleHandlerInterface> createBleHandler() {
  static std::shared_ptr<BleHandlerInterface> inst = std::make_shared<BleHandler>();
  return inst;
}
